...
async def get_event_discussions(
    ...
    event = (
        (await db.execute(select(models.Event).where(models.Event.id == event_id)))
        .scalars()
        .first()
    )
    if not event:
...
async def post_message(
    ...
    await db.execute(
        select(models.Message)
        .options(selectinload(models.Message.author))
        .where(models.Message.id == new_message.id)
    )
...
async def delete_message(
    ...
    stmt = (
        select(models.Message)
        .options(selectinload(models.Message.event))
        .where(models.Message.id == message_id, models.Message.event_id == event_id)
    )
    ...
    if not (
        ...
        or message.event.organizer_id == current_user.id
    ):
