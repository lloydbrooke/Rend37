...
@pytest.mark.asyncio
async def test_edit_event_requires_organizer(
    auth_client, db, seed_users, seed_community
):
    ...
    event = models.Event(
        ...
        organizer_id=seed_users[1].id,  # bob
    )
    db.add(event)
    await db.commit()
...
@pytest.mark.asyncio
async def test_delete_event_requires_organizer(
    auth_client, db, seed_users, seed_community
):
    event = models.Event(
        title="Not Mine",
...
async def test_register_for_event(auth_client, seed_event, db):
    resp = await auth_client.post(
        f"/events/{seed_event.id}/register", follow_redirects=False
    )
...
async def test_unregister_from_event(auth_client, seed_event, db, seed_users):
    ...
    db.add(
        models.Registration(
            user_id=seed_users[0].id, event_id=seed_event.id, status="registered"
        )
    )
    await db.commit()

    resp = await auth_client.post(
        f"/events/{seed_event.id}/unregister", follow_redirects=False
    )
    assert resp.status_code in (200, 302)

    result = await db.execute(
        select(models.Registration).filter_by(
            user_id=seed_users[0].id, event_id=seed_event.id
        )
    )
    assert result.scalars().first() is None
...
async def test_register_respects_capacity(auth_client, db, seed_users, seed_community):
    ...
    db.add(
        models.Registration(
            user_id=seed_users[1].id, event_id=event.id, status="registered"
        )
    )
    await db.commit()
    ...
    resp = await auth_client.post(
        f"/events/{event.id}/register", follow_redirects=False
    )
