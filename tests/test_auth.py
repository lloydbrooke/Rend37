...
async def test_register_page_renders(client):
    ...
    assert "register" in resp.text.lower() or "sign up" in resp.text.lower() or "form" in resp.text.lower()
...
async def test_register_duplicate_username(client):
    ...
    await client.post("/auth/register", data={
        "username": "taken", "email": "a@test.com", "password": "pass123"
    }, follow_redirects=False)
    # Try same username
    resp = await client.post("/auth/register", data={
        "username": "taken", "email": "b@test.com", "password": "pass123"
    })
    assert resp.status_code == 200
...
async def test_login_redirects_to_next(client, seed_users):
    resp = await client.post("/auth/login", data={
        "username": "alice", "password": "password123", "next": "/events"
    }, follow_redirects=False)
    assert resp.status_code == 302
